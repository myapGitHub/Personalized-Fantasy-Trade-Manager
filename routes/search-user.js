...
router.route("/").get(async (req, res) => {
  ...
  try {
    ...
    if (resultArr.length === 0)
      return res.render("pages/search-user", {
        empty: true,
...
router.route("/:userId").get(async (req, res) => {
  ...
  try {
    ...
    if (!profileStatus) {
      ...
      return res
        .status(400)
        .render("pages/Profiles/private", {
          error: "Error: fetching friend status",
        });
    }
...
router.route("/:userId/accept-request").post(async (req, res) => {
  ...
  try {
    ...
    const result = await userData.AcceptOrDeclineFriendRequest(
      currUser,
      sendToo,
      "accept"
    );
