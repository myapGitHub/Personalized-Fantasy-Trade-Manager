...
const calculateMaxes = ({ experience, age, weight, height, gender }) => {
  ...
  const benchMax =
    weight * multipliers.bench * genderMultiplier * ageMultiplier;
  const squatMax =
    weight * multipliers.squat * genderMultiplier * ageMultiplier;
  const deadliftMax =
    weight * multipliers.deadlift * genderMultiplier * ageMultiplier;
...
function checkValidRange(input, min, max, fieldName) {
  ...
  if (input < min || input > max) {
    throw `${fieldName} is required to be betweeen ${min} and ${max}`;
  }
}
...
export const signUp = async (
  ...
  checkValidRange(parseInt(height), 40, 300, "Height");
  // checks weight #TODO maybe add requirments
  weight = checkNum(parseInt(weight));
  checkValidRange(parseInt(weight), 10, 700, "Weight");
  // checks age #TODO maybe add requirments
  age = checkNum(parseInt(age));
  checkValidRange(parseFloat(age), 0, 120, "Age");
  // checks the gender
  gender = checkGender(gender);
  // checks experience
  level = checkExperience(level);
  const {
    benchMax: estBenchMax,
    squatMax: estSquatMax,
  ...
  if (!benchMax) {
    ...
    benchMax = checkNum(parseInt(benchMax));
    checkValidRange(parseInt(benchMax), 1, 1500, "Bench Max");
  }
...
export const userLogin = async (userIdParam, password) => {
  ...
  const userCollection = await users();
  const validUserIdFinder = await userCollection.findOne({
    userId: userIdParam.toLowerCase(),
  });
...
export const deleteAccount = async (userId) => {
  ...
  const userCollection = await users();
  const deleteProfile = await userCollection.deleteOne({
    userId: userId.toLowerCase(),
  });
  ...
  const deleteWorkouts = await workoutCollection.deleteMany({
    userId: userId.toLowerCase(),
  });
...
export const updateUserId = async (id, updateUserId) => {
  ...
  const userExists = await userCollection.findOne({
    userId: updateUserId.toLowerCase(),
  });
...
const checkCurrStatus = (status) => {
  ...
  if (typeof status !== "string" || status.trim().length === 0)
    throw "Status must be a string and not empty";
  if (status !== "public" && status !== "private")
    throw "Status must be 'public' or 'private'";
...
export const searchUser = async (currUserId, searchUserId) => {
  ...
  let exactMatchUser = await userCollection.findOne({
    userId: searchUserId.toLowerCase(),
  });
  if (exactMatchUser) {
...
export const getUserProfile = async (requstUserId) => {
  ...
  const userCollection = await users();
  const findUser = await userCollection.findOne({
    userId: requstUserId.toLowerCase(),
  });
...
export const sendFriendRequest = async (currUserId, requestUserId) => {
  ...
  const updatedFriendRequest = {
    ...currUser.friendRequest,
    [requestUserId]: true,
  };
  const updatedFriendInbox = {
    ...requestedUser.friendInbox,
    [currUserId]: true,
  };
...
const checkAction = (action) => {
  ...
  if (typeof action !== "string" || action.trim().length === 0)
    throw "has to be accept or decline";
  action = action.trim();
...
export const AcceptOrDeclineFriendRequest = async (
  ...
  if (action === "accept") {
    ...
    if (
      ...
      requestedUser.friendRequest[currUserId]
    ) {
  ...
  await userCollection.updateOne(
    ...
    {
      $set: {
        friendRequest: requestedUser.friendRequest,
...
export const getAllFriends = async (userId) => {
  ...
  if (!friends || typeof friends !== "object")
    throw "User doesnt have friend list";
...
export const getInboxFriends = async (userId) => {
  ...
  if (!friendInbox || typeof friendInbox !== "object")
    throw "User doesnt have friend list";
...
export const getAllFavoritedWorkouts = async (currUserId) => {
  ...
  for (const workoutId of favoriteWorkout) {
    try {
      const workoutObj = await workoutCollection.findOne({
