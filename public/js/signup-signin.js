    ...
    if (password === undefined || password === null) {
        ...
        if (password.length < 8) {
            ...
            if (!number || !capitalLetter || !specialChar) {
                errors.push("Password doesn't contain a capital letter, number, or special character");
            }
        }
    }
};
...
const checkFirstOrLastName = (name, errors, fieldName) => {
    ...
    if (name === undefined || name === null) {
        ...
        if (name.length < 2 || name.length > 25) {
            errors.push(`${fieldName} must be at least 2 characters long and not more than 25 characters`);
        }
